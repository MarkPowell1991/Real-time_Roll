    ...
    class PitchProcessor
    {
        ...
        static float SumAbsDiffHermite(float[] samples, float fOffset, int blockLen, int stepSize)
        {
            ...
            for (var idx = 0; idx < blockLen; idx += stepSize, count++)
            {
                ...
                value += Math.Abs(samples[idx] - InterpolateHermite(samples[offsetIdx - 1], samples[offsetIdx], samples[offsetIdx + 1], samples[offsetIdx + 2], offsetFrac));
            }
